...
(function() {
  ...
  const ScoreCalculator = {
    ...
    calculateRawScores: function(answers) {
      ...
      // Progressive sequence (1 to 20)
      const sequence = Array.from({ length: CONFIG.TOTAL_QUESTIONS }, (_, idx) => idx + 1);
      
      sequence.forEach((qId) => {
        const question = window.questions.find(q => q.id === qId);
        if (!question) return;
        const category = this.getCategoryFromQuestion(question);
    ...
    getCategoryFromQuestion: function(question) {
      const raw = String((question && question.category) || '').toLowerCase();
      if (raw.includes('data')) return 'DI';
      if (raw.includes('logical')) return 'LR';
      return 'NR';
    },
     ...
     */
    analyseTimePerformance: function(timeRemaining, answers) {
      const answeredCount = Math.max(1, Object.keys(answers).length);
      const timeUsed = CONFIG.DURATION_SECONDS - timeRemaining;
      const avgTimePerQuestion = timeUsed / answeredCount;
    ...
    analyzeTimePerformance: function(timeRemaining, answers) {
      return this.analyseTimePerformance(timeRemaining, answers);
    }
  };
